  ...
  return (
    <div>
      <HolographicPanel variant="blue" className="text-center">
        ...
        {/* Floating clothing holograms representation */}
        <div className="flex justify-center gap-6 mb-8">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="w-24 h-32 rounded-lg bg-gradient-to-br from-glow-blue/20 to-secondary/20 border border-glow-blue/30 float-slow flex items-center justify-center"
              style={{ animationDelay: `${i * 0.5}s` }}
            >
              <Shirt className="w-10 h-10 text-glow-blue/50" />
            </div>
          ))}
        </div>
